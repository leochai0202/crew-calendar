...
LATIN_PERSON_RE = re.compile(r"[A-Za-z][A-Za-z.\-']*(?:\s+[A-Za-z][A-Za-z.\-']*){1,7}\s*\([^)]*\)")
LATIN_BARE_PERSON_RE = re.compile(r"[A-Za-z][A-Za-z.\-']*(?:\s+[A-Za-z][A-Za-z.\-']*){1,7}")
LATIN_BARE_BEFORE_CJK_RE = re.compile(r"[A-Za-z][A-Za-z.\-']*(?:\s+[A-Za-z][A-Za-z.\-']*){1,7}(?=[\u4e00-\u9fff])")
...
def normalize_person_display_token(token: str) -> str:
    token = normalize_text(token)
    # 保留网页中的人员角色标记，例如 (R)、(P)、(B)、(P,B)、(T2,R)。
    # 角色属于人员信息，不能在标准化阶段丢弃。
...
def normalize_people_output(items: list) -> list:
    ...
    tagged_bases = set()
    for x in normalized:
        if ROLE_PAREN_RE.search(x):
            base = normalize_text(ROLE_PAREN_RE.sub("", x))
            if base:
    ...
    for x in normalized:
        ...
        if base in tagged_bases and not ROLE_PAREN_RE.search(x):
            continue
...
def split_compact_people_tokens(text: str) -> list:
    ...
    if re.fullmatch(r"[\u4e00-\u9fff]{2,3}[0-9A-Za-z]?", text):
        return [text]

    # 四字中文串既可能是复姓姓名，也可能是两个双字姓名紧连。
    # 已知人员直接保留；未知四字串继续进入拆分逻辑。
    if re.fullmatch(r"[\u4e00-\u9fff]{4}[0-9A-Za-z]?", text) and text in KNOWN_PEOPLE:
        return [text]
    ...
    candidates = dfs(0)
    if not candidates:
        surname_split = split_chinese_flight_people_by_surname(text)
        if surname_split and len(surname_split) > 1:
            return surname_split
        return [text]
    ...
    # 对“王伟李雷”“张三李四”这类 4 字紧凑双人名单，通用 DFS 可能
    # 把整串当作一个四字姓名；此时用姓氏边界结果纠正。
    if len(best) == 1 and re.fullmatch(r"[\u4e00-\u9fff]{4,15}", text):
        surname_split = split_chinese_flight_people_by_surname(text)
        if surname_split and len(surname_split) > 1:
            return surname_split
...
def parse_people_line_flight(line: str) -> list:
    ...
    # 纯英文姓名：允许带角色或不带角色。无角色英文姓名仅在整行就是姓名时接受，
    # 避免把普通英文说明误识别成人员。
    if LATIN_PERSON_RE.fullmatch(line) or LATIN_BARE_PERSON_RE.fullmatch(line):
        return normalize_people_output([line])
    ...
    if has_clear_delimiters(line):
        ...
        for p in parts:
            ...
            if not p or is_bad_title_text(p):
                continue
            if looks_like_person_token(p) or LATIN_BARE_PERSON_RE.fullmatch(p):
                valid.append(p)
...
def parse_mixed_people_line_flight(line: str) -> list:
    """
    按原始顺序解析中英文混排人员名单。

    支持：
      ALVARO CEMBRANOS GARCIA(R)徐宝建(R)段洋硕
      John Smith(R)王伟李雷(P)
      张三李四ALVARO CEMBRANOS GARCIA(R)
      ALVARO(R)（单词外籍姓名需带角色）

    规则：
    1. 带角色的英文姓名优先识别，不受同一行中文影响；
    2. 两个及以上英文单词组成的无角色姓名，可在整行或紧邻中文时识别；
    3. 英文姓名之间及中英文之间保持原始顺序；
    4. 剩余中文片段继续使用原有稳定的中文紧凑名单解析逻辑。
    """
    line = standardize_people_text(line)
    if not line:
        return []
    ...
    # 先收集所有可靠的英文姓名跨度。
    matches = list(LATIN_PERSON_RE.finditer(line))

    # 支持无角色的多词英文姓名紧邻中文，例如 John Smith王伟。
    occupied = [(m.start(), m.end()) for m in matches]
    for m in LATIN_BARE_BEFORE_CJK_RE.finditer(line):
    ...
    if not matches:
        return parse_people_line_flight(line)
    ...
    for m in matches:
        if m.start() > cursor:
            chunk = normalize_text(line[cursor:m.start()])
            if chunk:
                out.extend(parse_people_line_flight(chunk))

        token = normalize_person_display_token(m.group(0))
    ...
    if cursor < len(line):
        tail = normalize_text(line[cursor:])
        if tail:
            out.extend(parse_people_line_flight(tail))
...
def extract_people_lines_flight(card_text: str) -> list:
    ...
    for line in lines:
        ...
        # 中英文混排名单统一按原始顺序解析。英文姓名即使与中文同处一行也不能丢失。
        result = parse_mixed_people_line_flight(line)
        if result:
