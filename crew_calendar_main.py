...
def extract_people_lines_flight(card_text: str) -> list:
    ...
    for line in lines:
        ...
        # 优先提取外籍姓名 + 角色标记，例如 VERTIZ ROBLEDA IGNACIO(R)。
        latin_tagged = LATIN_PERSON_RE.findall(line)
        if latin_tagged:
            for t in latin_tagged:
...
def parse_generic_card(card_text: str, day_header: str, page_year: int, day_task_text: str, forced_kind: str):
    ...
    item = {
        ...
        "model": model,
        "reg": reg,
        "people_lines": segment_people_lists[i] if i < len(segment_people_lists) else [],
...
def extract_segment_people_from_day_block(day_block: str, segments: list) -> list:
    lines=[normalize_text(x) for x in day_block.splitlines() if normalize_text(x)]
    role_idx=[i for i,l in enumerate(lines) if l in ROLE_WORDS]
    if not role_idx:
        all_people=normalize_people_output(extract_people_lines_flight(day_block))
        return [all_people for _ in segments]
    groups=[]
    for n,idx in enumerate(role_idx):
        end=role_idx[n+1] if n+1<len(role_idx) else len(lines)
        block="\n".join(lines[idx:end])
        groups.append(normalize_people_output(extract_people_lines_flight(block)))
    result=[]
...
def parse_multi_segment_flight_items(day_header: str, day_block: str, page_year: int) -> list:
    """
    通用多段航班解析：
    适用于同一天 2-4 段甚至更多航班号先列出、航线时间在后面列出的格式。
    例如：
      9C8601
      9C8602
      上海浦东名古屋中部 11:00-13:25
    ...
    reg, model = extract_reg_and_model(day_block)
    segment_checkins = extract_segment_checkins_from_day_block(day_block, segments)
    segment_people_lists = extract_segment_people_from_day_block(day_block, segments)
    ...
    for i in range(count):
        ...
        # 修复：跨日判断必须按当前航段，不允许整天有一个 (+1) 就污染所有航段。
        # 例如 05/29 前两段 9C6391/9C6392 不跨日，第三段 9C8981 才跨日。
        ...
        checkin = segment_checkins[i] if i < len(segment_checkins) else {}
        checkin_time = checkin.get("checkin_time", "")
        checkin_place = checkin.get("checkin_place", "") or seg["dep_cn"]
        ...
        item = {
            ...
            "start_dt": start_dt,
            "end_dt": end_dt,
            "raw_card_text": normalize_text(day_block),
            "kind": "flight",
            "multi_segment": True,
        }
        items.append(item)
...
def run():
    ...
    logger.info("代码版本: multi-task-v11-segment-checkin-fixed")
    logger.info("=" * 60)
