...
VERSION = "flight-prep-free-v12.3-concise-group-template-20260701"
RISK_KEYWORDS = (
...
def feedback_text(profile: dict) -> str:
    ...
    text = "上一次飞行中教员对我优缺点的评价（作为PF/PM各取最近一次）：" + "；".join(parts) + "。"
    if rnp:
        text += f"补充讲评：{rnp}。"
    return text
...
def extract_manual_lists(section: dict, max_items: int) -> tuple[list[str], list[str]]:
    ...
    if not typical and table_records:
        typical = table_risk_items(table_records, max_items)
...
GROUP_CONCEPTS = (
    ...
    },
    {
        ...
        "core": "起飞和进近阶段应关注ATIS、管制及机场鸟情通报，加强外部观察；发现鸟群或发生鸟击后，根据飞机状态严格执行程序并及时报告。",
        "priority": 68,
    },
)
...
def validate_content(
    ...
    *,
    ...
    if detail:
        ...
        if "航路及连续运行：" not in content:
            errors.append("正文缺少航路及连续运行")
        if FLAP_CONFIGURATION_NOTE not in content:
            errors.append("正文缺少形态一时机提醒")
    if flights and not any(e.flight_number and e.flight_number in content for e in flights):
        errors.append("正文缺少航班号")
    for airport in airports:
        if detail:
            title = short_airport_name(airport) + "机场"
        else:
            title = group_airport_title(airport)
        if f"{title}典型不安全事件：" not in content:
            errors.append(f"正文漏掉{title}典型不安全事件")
        if f"{title}：" not in content:
            errors.append(f"正文漏掉{title}核心威胁")
...
def main() -> int:
    ...
    try:
        ...
        def build_group_content() -> str:
            sections: list[str] = []
            greeting = normalize_text(settings.get("greeting", ""))
            intro = group_personal_intro(profile, exp_records)
            ...
            sections.append(
                "个人对本次航班中识别的风险：\n"
                + group_risk_overview(flights, target, airports, risks, airport_threat_map)
            )

            for airport in airports:
                concise_items = group_typical_items(
                    risks.get(airport, []),
                ...
                risk_text = "\n".join(
                    ...
                    for i, item in enumerate(concise_items, start=1)
                )
                sections.append(f"{group_airport_title(airport)}典型不安全事件：\n{risk_text}")

            core_lines = ["核心威胁："]
            for airport in airports:
                canonical = canonical_airport_name(airport)
                if canonical in {"上海浦东", "丽江三义"}:
                    items = airport_core_items(airport, airport_threat_map.get(airport, []), target, context, detail=False)
                    concise_items = unique([concise_core_item(item) for item in items if concise_core_item(item)])[:3]
                else:
                    concise_items = group_core_sentences(airport_threat_map.get(airport, []), max_sentences=3)
                paragraph = "".join(concise_items)
                core_lines.append(f"{group_airport_title(airport)}：{paragraph}")
            sections.append("\n\n".join(core_lines))

            sections.append("航路及连续运行：" + group_route_control(target, flights))
            sections.append(FLAP_CONFIGURATION_NOTE)
            return "\n\n".join(s.strip() for s in sections if s.strip()).strip() + "\n"

        def build_detail_content() -> str:
            sections: list[str] = []
            greeting = normalize_text(settings.get("greeting", ""))
            ...
            risk_items = overall_risk_items(airports, target, weather_sentence, detail=True)
            ...
            general_lines.extend(
                ...
                for idx, item in enumerate(
                    general_control_items(target, airports, weather_sentence, detail=True),
                    start=1,
                )
            )
            sections.append("\n".join(general_lines))
        ...
        group_content = build_group_content()
        detail_content = build_detail_content()
        errors = [
