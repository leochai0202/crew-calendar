...
def extract_people_lines_flight(card_text: str) -> list:
    ...
    for line in lines:
        ...
        # 优先提取带角色标记的人名，例如 田鸿飞(R)张昌浩(R)段洋硕
...
def split_concat_airport_route(text: str):
    """
    解析类似：上海浦东名古屋中部 / 名古屋中部上海浦东。
    返回：dep_cn, arr_cn。解析不稳则返回空。
    """
    text = normalize_text(text)
    ...
    if not text or not re.fullmatch(r"[\u4e00-\u9fff]{4,30}", text):
        return "", ""

    # 优先按机场名完整匹配，避免把“上海浦东名古屋中部”拆错。
    ...
    temp = text
    for name in AIRPORT_NAMES:
        if name and name in temp:
            found.append((temp.find(name), name))
...
def parse_multi_segment_flight_items(day_header: str, day_block: str, page_year: int) -> list:
    ...
    checkin_time, checkin_place = extract_checkin(day_block)
    people_lines = normalize_people_output(extract_people_lines_flight(day_block))
    next_day_marker = has_next_day_marker(day_block)
    ...
    for i in range(count):
        ...
        if next_day_marker or (end_dt - start_dt).total_seconds() < 0:
            end_dt += timedelta(days=1)
...
def run():
    ...
    logger.info("代码版本: multi-task-v8-verified")
    logger.info("=" * 60)
