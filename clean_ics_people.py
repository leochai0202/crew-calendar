...
LATIN_WORD = r"[A-Za-zÀ-ÖØ-öø-ÿ][A-Za-zÀ-ÖØ-öø-ÿ'’.-]*"
LATIN_NAME = rf"{LATIN_WORD}(?:\s+{LATIN_WORD}){{0,7}}"
LATIN_PERSON_RE = re.compile(rf"{LATIN_NAME}(?:\([A-Z0-9,，、\s\*]+\))?")
LATIN_TAGGED_PERSON_RE = re.compile(rf"{LATIN_NAME}\([A-Z0-9,，、\s\*]+\)")
...
def looks_like_person_token(token: str) -> bool:
    ...
    if re.fullmatch(
        r"[\u4e00-\u9fff]{1,8}[0-9A-Za-z]?(?:\([A-Z0-9,，、\s\*]+\))?",
        token,
    ):
        return True

    return bool(re.fullmatch(rf"{LATIN_NAME}(?:\([A-Z0-9,，、\s\*]+\))?", token))
...
def split_or_fix_person_token(token: str) -> list[str]:
    ...
    # Preserve foreign names even when concatenated with Chinese names.
    matches = list(LATIN_TAGGED_PERSON_RE.finditer(token))
    if matches:
        out = [norm(m.group(0)) for m in matches]
        chars = list(token)
        for m in matches:
            for idx in range(m.start(), m.end()):
                chars[idx] = " "
        remaining = norm("".join(chars))
        if remaining:
            chunks = [c for c in re.split(r"[\s、，,;；/]+", remaining) if c]
            for chunk in chunks:
                out.extend(split_or_fix_person_token(chunk))
        return out

    # A standalone foreign name without a role tag is also valid.
    if re.fullmatch(LATIN_NAME, token):
        return [token]
